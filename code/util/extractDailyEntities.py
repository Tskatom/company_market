        ...
        with open(f) as tf:
            for line in tf:
                try:
                    ...
                    entities.setdefault(day,{
                        "urls":[],
                        "hashtags":[]})
                    #extract url information
                    mention = [m["expanded_url"] for m in tweet["entities"]["urls"]]
                    entities[day]["urls"].extend(mention)
                    #add hashtag infomation
                    hashtag = [h["text"].lower() for h in tweet["entities"]["hashtags"]]
                    entities[day]["hashtags"].extend(hashtag)
                except:
                    print 'error[%s]' % sys.exc_info()[0]
    return {"user": user, "entities": entities}
...
def main():
    ...
    for i in range(task_count):
        ...
        user = entities["user"]
        for day in entities["entities"]:
            if day not in out_files:
                out_files[day] = open(os.path.join(output, day), "w")
            out_files[day].write(json.dumps({"user": user, "entities": entities["entities"][day]}) + "\n")
